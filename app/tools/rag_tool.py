...
def search_pos_manuals(
  ...
  for attempt in range(1, max_retries + 1):
    try:
      ...
      logger.warning("Vector search error on attempt %d/%d: %s", attempt, max_retries, ex)
      last_error = ex
    ...
    for attempt in range(1, max_retries + 1):
      try:
        fallback_config = bigquery.QueryJobConfig(
            query_parameters=[
                bigquery.ScalarQueryParameter("search_term", "STRING", search_token)
            ]
        )
        fallback_rows = list(client.query(fallback_sql, job_config=fallback_config))
        if fallback_rows:
          best_row = fallback_rows[0]
          logger.info("SEARCH fallback successfully matched: %s", best_row.document_title)
        break
      except Exception as ex:
        logger.warning("Fallback search error on attempt %d/%d: %s", attempt, max_retries, ex)
        last_error = ex
        if attempt < max_retries:
          time.sleep(INITIAL_BACKOFF * (2 ** (attempt - 1)))
  ...
  logger.error("No troubleshooting guide matched or database unreachable: %s", last_error)
  return (
