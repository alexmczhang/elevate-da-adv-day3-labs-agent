...
DEFAULT_BIGTABLE_MCP_URL = os.getenv(
    "BIGTABLE_MCP_URL",
    "https://mcp-toolbox-bigtable-604493420076.us-central1.run.app"
)
IMPERSONATION_SA = "cymbal-sa-data@da-c3-group3.iam.gserviceaccount.com"


def get_bigtable_mcp_oidc_token(target_audience: Optional[str] = None) -> str:
    ...
    try:
        ...
        logger.warning("Failed to generate impersonated ID token: %s. Falling back to gcloud identity token.", e)
        try:
            ...
            raise RuntimeError(f"Could not generate GCP OIDC Token: {fallback_err}") from e
...
def read_cashier_realtime_metrics(
    ...
    with urllib.request.urlopen(mcp_req, timeout=15) as resp:
        ...
        if not content:
            return {"status": "NO_RECORDS_FOUND", "message": f"No alerts found for {clean_store}#{clean_cashier}"}
        ...
        parsed = {
            ...
            "cashier_1h_manual_override_count": row.get("cashier_1h_manual_override_count"),
            "cashier_1h_promo_count": row.get("cashier_1h_promo_count"),
            "cashier_1h_txn_count": row.get("cashier_1h_txn_count"),
            "cashier_1h_avg_discount_pct": _decode_double_if_b64(row.get("cashier_1h_avg_discount_pct")),
            "cashier_1h_promo_rate": _decode_double_if_b64(row.get("cashier_1h_promo_rate")),
            "cashier_1h_total_discount_usd": _decode_double_if_b64(row.get("cashier_1h_total_discount_usd")),
        ...
        if parsed.get("cashier_1h_txn_count") and parsed["cashier_1h_txn_count"] > 0:
            parsed["cashier_1h_override_rate"] = round(
                (parsed.get("cashier_1h_manual_override_count", 0) / parsed["cashier_1h_txn_count"]) * 100.0,
                2
            )
        return parsed
